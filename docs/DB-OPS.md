...
- agy는 tool 권한 프롬프트가 뜰 수 있음 — Orca 터미널에서 승인하거나 필요 시 재실행 옵션 조정. **최초 실행 시 "폴더 신뢰" 대화상자가 프롬프트를 삼킴** — 신뢰 승인 후 미션을 다시 보내야 함.

### ⚠️ 에이전트별 신뢰도 교훈 (2026-08-16)

- **agy(Gemini Flash)의 "검증" 결과는 신뢰 불가**: stay 100건 검증 미션에서 100% verified·폐업 0건 반환 — 근거 URL이 예약사이트 홈페이지 루트 수준으로, 실검색 아닌 환각 패턴. **검증(폐업/영업 판정) 작업은 Claude 세션이나 하이쿠 웨이브로만** 수행할 것.
- agy의 **수집**(유명 스폿 리스트업)은 사용 가능 — 실존 유명 업장 위주로 뽑는 경향. 단 출처 URL 일부 위조 가능성은 감안.
- 판정 기준: 검증 결과에서 verified 비율이 90%+ 이고 unknown이 거의 없으면 환각 의심 (정상 분포는 open 50~60% / unknown 30~40% / closed 5~10%).
