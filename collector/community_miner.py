...
def _clean_comm_cand(s: str) -> str:
    if not s:
        return ""
    c = s.strip()
...
def extract_list_items(html_text: str):
    ...
    for tag in hashtags:
        clean_tag = _clean_comm_cand(tag)
        if 2 <= len(clean_tag) <= 10 and not any(w in clean_tag for w in _COMMUNITY_STOP_WORDS):
            candidates.add(clean_tag)
